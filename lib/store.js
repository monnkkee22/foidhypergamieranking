...
import { sanitizeData } from "./rankingUtils";
...
export async function getData() {
  if (hasBlobConfig()) {
    // Schlägt das Lesen fehl (z.B. kurzer Netzwerkfehler), wird der Fehler
    // bewusst weitergereicht. Die Standarddaten dürfen NIEMALS als Reaktion
    // auf einen Lesefehler in den Blob geschrieben werden, sonst würden alle
    // gespeicherten Rankings überschrieben.
    const stored = await readBlobData();
    if (stored) return sanitizeData(stored) || DEFAULT_DATA;

    // Blob existiert noch nicht (allererster Aufruf): Standarddaten anlegen.
  ...
  const local = sanitizeData(readLocalFile());
